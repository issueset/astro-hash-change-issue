...
class H extends HTMLElement {
  constructor() {
    (super(),
      ...
      (this.tocHeadingSelector = `h1#${m},:where(${[...Array.from({ length: 1 + this.maxH - this.minH }).map((e, n) => `h${this.minH + n}`)].join()})[id]`),
      (this.onIdle = (e) =>
        (window.requestIdleCallback || ((n) => setTimeout(n, 1)))(e)),
      (this.init = () => {
        const e = [...this.querySelectorAll("a")],
          n = (t) => t.matches(this.tocHeadingSelector),
          i = (t) => {
            if (!t) return null;
            const h = t;
            for (; t; ) {
              if (t.matches(".sl-markdown-content, main > *"))
                return document.getElementById(m);
              if (n(t)) return t;
              const s = t.querySelector(this.tocHeadingSelector);
              if (s) return s;
            ...
            return i(h.parentElement);
          },
          c = (t) => {
            for (const { isIntersecting: h, target: s } of t) {
              if (!h) continue;
              const o = i(s);
              if (!o) continue;
              const l = e.find(
                (g) => g.hash === "#" + encodeURIComponent(o.id),
              );
              if (l) {
                this.current = l;
                break;
        ...
        window.addEventListener("resize", () => {
          (r && (r.disconnect(), (r = void 0)),
            clearTimeout(d),
            (d = setTimeout(() => this.onIdle(u), 200)));
        });
      }),
...
customElements.define("starlight-toc", H);
export { H as S };
