import {
  ...
  R as RenderUndefinedEntryError,
  C as escape$1,
  u as unescapeHTML,
  b as renderTemplate,
  D as renderUniqueStylesheet,
  E as renderScriptElement,
  G as createHeadAndContent,
  ...
  return (function e(u) {
    ...
    function g() {
      if ("object" == typeof d.node && null !== d.node) {
        ((d.keys && d.node_ === d.node) || (d.keys = l(d.node)),
    ...
    const m = t(d, d.node);
    if (
      (void 0 !== m && d.update && d.update(m),
      f.before && f.before(d, d.node),
      !h)
    )
      return d;
...
var j = class {
  ...
  get(e) {
    ...
    for (let o = 0; t && o < e.length; o++) {
      const n = e[o];
      if (!p.call(t, n) || (!this.#t.includeSymbols && "symbol" == typeof n))
        return;
      t = t[n];
    }
    return t;
...
class ImmutableDataStore {
  ...
  entries(collectionName) {
    const collection =
      this._collections.get(collectionName) ?? /* @__PURE__ */ new Map();
  ...
  static async fromModule() {
    try {
      const data = await import("./_astro_data-layer-content-BnMGPDyO.js");
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (collection in contentCollectionToEntryMap) {
      ...
      const { default: imageAssetMap } =
        await import("./content-assets-DBDm90cm.js");
      const result = [];
    ...
    let entries = [];
    if (
      !Object.assign(__vite_import_meta_env__, {})?.DEV &&
...
function createGetEntry({
  ...
  return async function getEntry(collectionOrLookupObject, lookup) {
    ...
    if (store.hasCollection(collection)) {
      ...
      const { default: imageAssetMap } =
        await import("./content-assets-DBDm90cm.js");
      entry2.data = updateImageReferencesInData(
        entry2.data,
        entry2.filePath,
        imageAssetMap,
      );
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import("./_astro_assets-BrHcXheo.js").then(
    (n) => n._,
  );
  ...
  return html.replaceAll(CONTENT_LAYER_IMAGE_REGEX, (full, imagePath) => {
    ...
    return Object.entries({
      ...
      .map(([key, value]) => (value ? `${key}="${escape$1(value)}"` : ""))
      .join(" ");
  });
}
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } =
        await import("./content-modules-DryJ95yv.js");
...
const starlightConfig = {
  ...
  sidebar: [
    ...
    },
    {
      ...
      translations: {},
      collapsed: false,
      autogenerate: { directory: "reference", attrs: {} },
    },
  ],
  head: [],
  lastUpdated: false,
...
function starlightI18nSchema() {
  return objectType({
    ...
    "search.cancelLabel": stringType().describe(
      "Text for the “Cancel” button that closes the search modal.",
    ),
    "search.devWarning": stringType().describe(
      "Warning displayed when opening the Search in a dev environment.",
    ),
    "themeSelect.accessibleLabel": stringType().describe(
      "Accessible label for the theme selection dropdown.",
    ),
    "themeSelect.dark": stringType().describe("Name of the dark color theme."),
    "themeSelect.light": stringType().describe(
      "Name of the light color theme.",
    ),
    "themeSelect.auto": stringType().describe(
      "Name of the automatic color theme that syncs with system preferences.",
    ),
    "languageSelect.accessibleLabel": stringType().describe(
      "Accessible label for the language selection dropdown.",
    ),
    "menuButton.accessibleLabel": stringType().describe(
    ...
    "tableOfContents.onThisPage": stringType().describe(
      "Title for the table of contents component.",
    ),
    ...
    "page.lastUpdated": stringType().describe(
    ...
    "page.nextLink": stringType().describe(
    ...
    "404.text": stringType().describe(
      "Text shown on Starlight’s default 404 page",
    ),
    ...
    "aside.caution": stringType().describe(
      "Text shown on the warning aside variant",
    ),
    "aside.danger": stringType().describe(
      "Text shown on the danger aside variant",
    ),
    "fileTree.directory": stringType().describe(
      "Label for the directory icon in the file tree component.",
    ),
    ...
    "heading.anchorLabel": stringType().describe(
      "Label for anchor links in Markdown content.",
    ),
  }).partial();
}
...
const cs = {
  ...
  "page.nextLink": "Další",
  "page.draft":
    "Tento obsah je koncept a nebude zahrnutý v produkčním sestavení.",
  "404.text":
    "Stránka nenalezena. Zkontrolujte adresu nebo zkuste použít vyhledávač",
  "aside.note": "Poznámka",
  "aside.tip": "Tip",
  ...
  "pagefind.alt_search":
    "Žádné výsledky pro [SEARCH_TERM]. Namísto toho zobrazuji výsledky pro: [DIFFERENT_TERM]",
  "pagefind.search_suggestion":
    "Žádný výsledek pro [SEARCH_TERM]. Zkus nějaké z těchto hledání:",
...
const es = {
  ...
  "page.draft":
    "Este contenido es un borrador y no se incluirá en las versiones de producción.",
  "404.text":
    "Página no encontrada. Verifica la URL o intenta usar la barra de búsqueda.",
  "aside.note": "Nota",
  "aside.tip": "Consejo",
  ...
  "pagefind.alt_search":
    "Ningún resultado para [SEARCH_TERM]. Mostrando resultados para: [DIFFERENT_TERM]",
  "pagefind.search_suggestion":
    "Ningún resultado para [SEARCH_TERM]. Prueba alguna de estas búsquedas:",
...
const ca = {
  ...
  "page.draft":
    "Aquest contingut és un esborrany i no s'inclourà en les versions de producció.",
  "404.text":
    "Pàgina no trobada. Comprova la URL o intenta utilitzar la barra de cerca.",
  "aside.note": "Nota",
  "aside.tip": "Consell",
  ...
  "pagefind.alt_search":
    "Cap resultat per a [SEARCH_TERM]. Mostrant resultats per a: [DIFFERENT_TERM]",
  "pagefind.search_suggestion":
    "Cap resultat per a [SEARCH_TERM]. Prova alguna d’aquestes cerques:",
...
const de = {
  ...
  "page.draft":
    "Dieser Inhalt ist ein Entwurf und wird nicht in den Produktions-Builds enthalten sein.",
  "404.text":
    "Seite nicht gefunden. Überprüfe die URL oder nutze die Suchleiste.",
  "aside.note": "Hinweis",
  "aside.tip": "Tipp",
...
const ja = {
  ...
  "page.nextLink": "次へ",
  "page.draft":
    "このコンテンツは下書きです。プロダクションビルドには含まれません。",
  "404.text":
    "ページが見つかりません。 URL を確認するか、検索バーを使用してみてください。",
  "aside.note": "ノート",
  "aside.tip": "ヒント",
  "aside.caution": "注意",
  "aside.danger": "危険",
...
const pt = {
  ...
  "page.draft":
    "Esse conteúdo é um rascunho e não será incluído em builds de produção.",
  "404.text":
    "Página não encontrada. Verifique o URL ou tente usar a barra de pesquisa.",
  "aside.note": "Nota",
  "aside.tip": "Dica",
...
const fa = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "صفحه یافت نشد. لطفاً URL را بررسی کنید یا از جستجو استفاده نمایید.",
  "aside.note": "یادداشت",
  "aside.tip": "نکته",
...
const fi = {
  ...
  "page.draft":
    "Tämä sisältö on luonnos eikä sitä sisällytetä tuotantoversioon.",
...
const gl = {
  ...
  "i18n.untranslatedContent":
    "Este contido aínda non está dispoñible no teu idioma.",
  "page.editLink": "Editar páxina",
  "page.lastUpdated": "Última actualización:",
  "page.previousLink": "Anterior",
  "page.nextLink": "Seguinte",
  "page.draft":
    "Este contido é un borrador e non se incluirá nas versións de producción.",
  "404.text":
    "Páxina non atopada. Comproba a URL ou intenta usar a barra de busca.",
  "aside.note": "Nota",
  "aside.tip": "Consello",
  "aside.caution": "Precaución",
  "aside.danger": "Perigo",
  ...
  "pagefind.one_result": "[COUNT] resultado para: [SEARCH_TERM]",
  "pagefind.alt_search":
    "Ningún resultado para [SEARCH_TERM]. Amósanse resultados para: [DIFFERENT_TERM]",
  "pagefind.search_suggestion":
    "Ningún resultado para [SEARCH_TERM]. Proba algunha destas buscas:",
  "pagefind.searching": "Buscando [SEARCH_TERM]...",
  "heading.anchorLabel": "Sección titulada «{{title}}»",
};
...
const id = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "Halaman tidak ditemukan. Cek kembali kolom URL atau gunakan fitur pencarian.",
  "aside.note": "Catatan",
  "aside.tip": "Tips",
...
const it = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "Pagina non trovata. Verifica l'URL o prova a utilizzare la barra di ricerca.",
  "aside.note": "Nota",
  "aside.tip": "Consiglio",
...
const da = {
  ...
  "page.nextLink": "Næste",
  "page.draft":
    "Indholdet er en kladde og vil ikke blive inkluderet i produktions versioner.",
...
const tr = {
  ...
  "404.text":
    "Sayfa bulunamadı. URL'i kontrol edin ya da arama çubuğunu kullanmayı deneyin.",
  "aside.note": "Not",
  "aside.tip": "İpucu",
...
const nb = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "Siden ble ikke funnet. Sjekk URL-en eller prøv å bruke søkefeltet.",
  "aside.note": "Merknad",
  "aside.tip": "Tips",
...
const ko = {
  ...
  "page.draft":
    "이 콘텐츠는 아직 초안 상태이며, 최종 빌드에는 포함되지 않습니다.",
  "404.text":
    "페이지를 찾을 수 없습니다. URL을 다시 확인해보거나 검색을 사용해보세요.",
  "aside.note": "참고",
  "aside.tip": "팁",
  "aside.caution": "주의",
  "aside.danger": "위험",
...
const sv = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "Sidan hittades inte. Kontrollera URL:n eller testa att använda sökfältet.",
  "aside.note": "Note",
  "aside.tip": "Tip",
  "aside.caution": "Caution",
  "aside.danger": "Danger",
...
const ro = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "Pagina nu a fost găsită. Verifică adresa URL sau încercă să folosești bara de căutare.",
  "aside.note": "Mențiune",
  "aside.tip": "Sfat",
...
const ru = {
  ...
  "page.draft":
    "Этот контент является черновиком и не будет добавлен в продакшен-сборки.",
  "404.text":
    "Страница не найдена. Проверьте URL или используйте поиск по сайту.",
  "aside.note": "Заметка",
  "aside.tip": "Совет",
...
const vi = {
  ...
  "search.devWarning":
    "Chức năng tìm kiếm chỉ có sẵn trong các phiên bản thật.\nHãy thử xây dựng và xem trước trang web để kiểm tra.",
  ...
  "page.nextLink": "Tiếp",
  "page.draft":
    "Nội dung này là bản nháp và sẽ không được đưa vào các phiên bản thật.",
  "404.text":
    "Không tìm thấy trang. Hãy kiểm tra lại URL hoặc thử dùng thanh tìm kiếm.",
  "aside.note": "Ghi chú",
  "aside.tip": "Mẹo",
  "aside.caution": "Chú ý",
...
const uk = {
  ...
  "page.nextLink": "Далі",
  "page.draft":
    "Цей контент є чернеткою і не буде включений до виробничих збірок.",
  "404.text":
    "Сторінку не знайдено. Перевірте URL або спробуйте скористатися пошуком.",
  "aside.note": "Заувага",
  "aside.tip": "Порада",
...
const hi = {
  ...
  "page.draft":
    "This content is a draft and will not be included in production builds.",
  "404.text":
    "यह पृष्ठ नहीं मिला। URL जांचें या खोज बार का उपयोग करने का प्रयास करें।",
  "aside.note": "टिप्पणी",
  "aside.tip": "संकेत",
...
const sk = {
  ...
  "page.draft":
    "Tento obsah je koncept a nebude zahrnutý do produkčných zostavení.",
  "404.text":
    "Stránka nenájdená. Skontrolujte URL alebo skúste použiť vyhľadávacie pole.",
  "aside.note": "Poznámka",
  "aside.tip": "Tip",
...
const lv = {
  ...
  "page.draft":
    "Šis saturs ir melnraksts un netiks iekļauts ražošanas kompilācijās.",
  "404.text":
    "Lapa nav atrasta. Pārbaudiet URL vai mēģiniet izmantot meklēšanas joslu.",
  "aside.note": "Piezīme",
  "aside.tip": "Padoms",
...
const hu = {
  ...
  "page.draft":
    "Ez a tartalom még vázlat, így nem lesz benne a production build-ben.",
  "404.text":
    "Az oldal nem található. Nézd meg az URL-t vagy használd a keresőt.",
  ...
  "pagefind.alt_search":
    "Erre a kifejezésre nincs találat: [SEARCH_TERM]. Találatok mutatása erre: [DIFFERENT_TERM]",
  "pagefind.search_suggestion":
    "Erre a kifejezésre nincs találat: [SEARCH_TERM]. Próbáld meg ezek közül az egyiket:",
...
const el = {
  ...
  "page.draft":
    "Αυτό το περιεχόμενο είναι πρόχειρο και δεν θα περιλαμβάνεται στα builds παραγωγής.",
  "404.text":
    "Η σελίδα δεν βρέθηκε. Έλεγξε το URL ή χρησιμοποίησε τη γραμμή αναζήτησης.",
  "aside.note": "Σημείωση",
  "aside.tip": "Συμβουλή",
...
async function createTranslationSystem(
  config,
  userTranslations,
  pluginTranslations = {},
) {
...
function getCollectionPathFromRoot(collection, { root, srcDir }) {
  return srcDir.replace(root, "") + "content/" + collection;
}
  ...
  const userTranslations = Object.fromEntries(
    ...
    (await getCollection("i18n")).map(({ id, data, filePath }) => {
      const lang = !filePath
        ? id
        : stripExtension(
            stripLeadingSlash(filePath.replace(i18nCollectionPathFromRoot, "")),
          );
      return [lang, data];
    }),
  );
...
export {
  ...
  stripTrailingSlash as b,
  stripLeadingSlash as c,
  stripHtmlExtension as d,
  ensureHtmlExtension as e,
  ensureTrailingSlash as f,
  getCollection as g,
  getCollectionPathFromRoot as h,
  pickLang as i,
  stripLeadingAndTrailingSlashes as j,
